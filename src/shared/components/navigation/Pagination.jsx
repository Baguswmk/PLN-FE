...
const Pagination = ({
  ...
  const pageOptions = [...pageSizeOptions];
      ...
      <div className="hidden sm:flex items-center justify-center gap-2 mt-4 flex-wrap">
        ...
        {pages.map((page, idx) => (
          <Button
            ...
            onClick={() => typeof page === "number" && onPageChange(page)}
            disabled={typeof page !== "number" || isLoading}
            className="cursor-pointer disabled:cursor-not-allowed min-w-9 dark:border-gray-700 dark:bg-slate-800 dark:hover:bg-gray-700 dark:text-gray-200"
            aria-label={
              typeof page === "number" ? `Page ${page}` : "More pages"
            }
            aria-current={page === currentPage ? "page" : undefined}
